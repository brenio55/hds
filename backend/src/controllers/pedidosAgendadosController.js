...
class PedidosConsolidadosController {
  static async findAll(req, res) {
    try {
      console.log('========== INÍCIO - PEDIDOS AGENDADOS CONTROLLER ==========');
      console.log('Requisição recebida em /api/pedidos-agendados');
      ...
      if (tipo && id) {
            ...
            case 'compra':
              ...
              if (pedido) {
                ...
                const faturamentos = await db.query(
                  'SELECT * FROM faturamento WHERE id_type = $1 AND id_number = $2 AND agendamento = TRUE',
                  ['compra', id]
                );
                ...
                if (faturamentos.rows.length > 0) {
                  ...
                  tipoDePagamento = ultimoFaturamento.pagamento;
                  detalhesDePagamento = ultimoFaturamento.detalhes_pagamento;
                  nf = ultimoFaturamento.nf;
                }
                ...
                pedido = {
                  ...
                  detalhes_de_pagamento: detalhesDePagamento,
                  nf: nf
                };
              }
              break;
            ...
            case 'locacao':
              ...
              case 'servico':
                ...
                if (pedido) {
                  ...
                  // Adicionado declaração de valorTotal
                  let valorTotal = parseFloat(pedido.valor_total) || 0; // Adicione esta linha
                  ...
                  // Removida referência a cliente que não existe em serviços
                  pedido = {
                    ...pedido,
                    tipo: 'servico',
                    fornecedor: fornecedor,
        ...
        if (pedido) {
          ...
          const pedidoFormatado = {
            ...
            tipo_de_pagamento: tipoDePagamento || 'Não informado',
            detalhes_de_pagamento: detalhesDePagamento || 'Não informado',
            nf: nf || "Não informado"
          };
      ...
      try {
        ...
        const resultCompra = await db.query(`
          ...
          INNER JOIN faturamento f ON f.id_type = 'compra' AND f.id_number = pc.id
          WHERE f.agendamento = TRUE
          ORDER BY pc.created_at DESC
        `);
        pedidosCompra = resultCompra.rows;
        console.log(`Pedidos de compra encontrados: ${pedidosCompra.length}`);
      } catch (errorCompra) {
      ...
      try {
        ...
        const resultLocacao = await db.query(`
          SELECT pl.* 
          FROM pedido_locacao pl
          INNER JOIN faturamento f ON f.id_type = 'locacao' AND f.id_number = pl.id
          WHERE f.agendamento = TRUE
          ORDER BY pl.created_at DESC
        `);
        pedidosLocacao = resultLocacao.rows;
        console.log(`Pedidos de locação encontrados: ${pedidosLocacao.length}`);
      } catch (errorLocacao) {
      ...
      try {
        ...
        const resultServicos = await db.query(`
          SELECT s.* 
          FROM servico s
          INNER JOIN faturamento f ON f.id_type = 'servico' AND f.id_number = s.id
          WHERE f.agendamento = TRUE
          ORDER BY s.created_at DESC
        `);
        servicos = resultServicos.rows;
      ...
      try {
        ...
        const resultFaturamentos = await db.query('SELECT * FROM faturamento WHERE agendamento = TRUE');
      ...
      try {
        ...
        pedidosCompraFormatados = pedidosCompra
          ...
          .map(pedido => {
            try {
              if (!pedido.materiais) {
                console.error(`Pedido ${pedido.id} sem campo materiais`);
                return {
                  ...pedido,
                  error: true,
                  error_msg: 'Estrutura de materiais inválida'
                };
              }
              ...
              if (faturamento) {
                ...
                valorTotal = parseFloat(faturamento.valor_total_pedido) || valorTotal;
                valorAFaturar = Math.max(0, valorTotal - valorFaturado);
                tipoDePagamento = faturamento.pagamento;
                detalhesDePagamento = faturamento.detalhes_pagamento;
                nf = faturamento.nf;
              }
      ...
      try {
        ...
        servicosFormatados = servicos
          ...
          .map(servico => {
            try {
              // Adicionado declaração de valorTotal
              let valorTotal = parseFloat(servico.valor_total) || 0;
              ...
              // Corrigido para usar servico em vez de pedido
        ...
        console.error('ERRO ao formatar serviços:', errorFormatServicos.message);
        servicosFormatados = [];
      }
        ...
        pedidosSimplificados = todosPedidos.map(pedido => {
          try {
            const pedidoSeguro = {
              ...
              valor_faturado: pedido.valor_faturado || 0,
              valor_a_faturar: pedido.valor_a_faturar || 0,
              tipo_de_pagamento: pedido.tipo_de_pagamento || 'Não informado',
              detalhes_de_pagamento: pedido.detalhes_de_pagamento || 'Não informado',
              nf: pedido.nf || 'Não informado'
            };
